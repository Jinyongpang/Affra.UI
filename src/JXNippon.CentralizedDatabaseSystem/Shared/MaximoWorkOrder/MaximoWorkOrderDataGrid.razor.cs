...
namespace JXNippon.CentralizedDatabaseSystem.Shared.MaximoWorkOrder
{
    ...
    {
        private RadzenDataGrid<DailyMaximoWorkOrder> grid;
        private IEnumerable<DailyMaximoWorkOrder> items;
        ...
        private async Task LoadDataAsync(LoadDataArgs args)
        {
            ...
            if (CommonFilter != null)
            {
                ...
                {
                    query = query.Where(x => x.WorkOrderTypeName.ToUpper().Contains(CommonFilter.Search.ToUpper()));
                }
            ...
            var response = await query
                ...
                .ToQueryOperationResponseAsync<DailyMaximoWorkOrder>();
        ...
        private IGenericService<DailyMaximoWorkOrder> GetGenericService(IServiceScope serviceScope)
        {
            return serviceScope.ServiceProvider.GetRequiredService<IUnitGenericService<DailyMaximoWorkOrder, ICentralizedDatabaseSystemUnitOfWork>>();
        }

        private async Task ShowDialogAsync(DailyMaximoWorkOrder data, int menuAction, string title)
        {
            ...
            {
                response = await DialogService.OpenAsync<MaximoWorkOrderDialog>(title,
