...
{
    public partial class DecimalMonthlyAvailabilityAndReliabilityDataItem<TItem>
        where TItem : class
    {
        ...
        {
            if (IsEditable && await UserService.CheckHasPermissionAsync(null, new Permission { Name = nameof(FeaturePermission.AvailabilityAndReliability), HasReadPermissoin = true, HasWritePermission = true }))
            {
                isEditing = true;
