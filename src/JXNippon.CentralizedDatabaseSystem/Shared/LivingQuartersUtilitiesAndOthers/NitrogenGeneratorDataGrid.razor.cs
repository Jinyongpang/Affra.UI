...
namespace JXNippon.CentralizedDatabaseSystem.Shared.LivingQuartersUtilitiesAndOthers
{
    ...
    {
        private RadzenDataGrid<DailyNitrogenGenerator> grid;
        private IEnumerable<DailyNitrogenGenerator> items;
        ...
        private IGenericService<DailyNitrogenGenerator> GetGenericService(IServiceScope serviceScope)
        {
            return serviceScope.ServiceProvider.GetRequiredService<IUnitGenericService<DailyNitrogenGenerator, ICentralizedDatabaseSystemUnitOfWork>>();
        }

        private async Task ShowDialogAsync(DailyNitrogenGenerator data, int menuAction, string title)
        {
            ...
            {
                response = await DialogService.OpenAsync<NitrogenGeneratorDialog>(title,
