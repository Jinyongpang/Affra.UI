...
namespace JXNippon.CentralizedDatabaseSystem.Shared.ProducedWaterTreatmentSystemManagement
{
    public partial class ProducedWaterTreatmentSystemManagementDataGrid
    {
        ...
        private async Task ShowDialogAsync(DailyProducedWaterTreatmentSystem data, int menuAction, string title)
        {
            ...
            {
                response = await DialogService.OpenAsync<ProducedWaterTreatmentSystemDialog>(title,
                ...
                if (response == true)
                {
                    try
                    {
                        ...
                        data.LastSkimmingDate = data.LastSkimmingDate.ToUniversalTime();
